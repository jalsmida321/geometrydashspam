...
/**
 * CategoryPage component for displaying games in a specific category
 */
const CategoryPage: React.FC = () => {
  ...
  const handleSearch = (query: string, filter?: GameFilter) => {
    ...
    // TODO: Handle filter parameter when implementing advanced filtering
  };
  ...
  return (
    <>
      <SEOHead
        ...
        keywords={[category.name.toLowerCase(), 'games', 'online', 'free', 'browser games', ...category.name.toLowerCase().split(' ')]}
        category={category}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {categories.filter(c => c.id !== category.id).slice(0, 3).map(cat => (
                    <button
                      key={cat.id}
